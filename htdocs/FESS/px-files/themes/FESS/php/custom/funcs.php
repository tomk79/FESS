...
class pxtheme_custom_funcs{
	...
	public function mk_html_gnavi( $top_page_id, $depth = 0 ){
		...
		if( count($local_children) ){
			...
			foreach( $local_children as $local_child_page_id ){
				...
				$rtn .= '<li>';
				$rtn .= $this->px->mk_link($local_child_page_info['path']);
	...
	public function mk_breadcrumb(){
		...
		if( $page_info['id'] == '' ){
			//  ホームの場合
			return '<ul><li><strong>'.htmlspecialchars($this->px->site()->get_page_info('','title_breadcrumb')).'</strong></li></ul>';
		}
		...
		$rtn = '';
		$rtn .= '<ul>';
		$rtn .= '<li><a href="'.htmlspecialchars($this->px->href('')).'">'.htmlspecialchars($this->px->site()->get_page_info('','title_breadcrumb')).'</a></li>';
		foreach( $array_breadcrumb as $page_id ){
			$linkto_page_info = $this->px->site()->get_page_info($page_id);
			$href = $this->px->href($linkto_page_info['path']);
			if( $href == $current_path ){
				$rtn .= '<li> &gt; '.htmlspecialchars($linkto_page_info['title_breadcrumb']).'</li>';
			}else{
				$rtn .= '<li> &gt; <a href="'.htmlspecialchars($href).'">'.htmlspecialchars($linkto_page_info['title_breadcrumb']).'</a></li>';
			}
		}
		$rtn .= '<li> &gt; <strong>'.htmlspecialchars($page_info['title_breadcrumb']).'</strong></li>';
		$rtn .= '</ul>';
		return $rtn;
	}//mk_breadcrumb()
